...
def read_keypoints(json_input, size, random_drop_prob=0, remove_face_labels=False, basic_point_only=False):
    ...
    '''
    if json_input.rfind('target') != -1:
        pose_pts_valid = np.zeros((25, 2))
        valid = (pose_pts[:, 2] > 0.01)
        pose_pts_valid[valid, :] = pose_pts[valid, :2]
        pos = np.sum(pose_pts_valid, axis=0) / np.sum(valid==True)
        pos_x = pos[0]
        pos_y = pos[1]
        translation_x = 960 - pos_x
        translation_y = 540 - pos_y
