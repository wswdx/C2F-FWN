...
# class for discriminator of layout GAN
class Vid2VidModelD(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.output_nc = opt.output_nc_1        

        # define networks        
        # single image discriminator
        netD_input_nc = opt.input_nc_T_1 + opt.input_nc_S_1 + opt.label_nc_1
        ...
        netD_input_nc = opt.label_nc_1 * opt.n_frames_D + 2 * (opt.n_frames_D-1)     
        ...
        self.criterionLO = networks.PixelwiseSoftmaxLoss()
    ...
    def forward(self, scale_T, tensors_list, dummy_bs=0):
        ...
        if scale_T > 0:
            real_slo, fake_slo, flow_ref, conf_ref, _ = tensors_list
            _, _, _, self.height, self.width = real_slo.size()
            loss_D_T_real, loss_D_T_fake, loss_G_T_GAN, loss_G_T_GAN_Feat = self.compute_loss_D_T(real_slo, fake_slo, 
        ...
        real_slo, fake_slo, fake_slo_raw, fake_slo_ls, fake_slo_raw_ls, real_input_T, real_input_S, real_slo_prev, fake_slo_prev, flow, weight, flow_ref, conf_ref = tensors_list
        real_input_T = real_input_T.expand(-1, real_input_S.size()[1], -1, -1, -1)
        real_slo, fake_slo, fake_slo_raw, fake_slo_ls, fake_slo_raw_ls, real_input_T, real_input_S, real_slo_prev, fake_slo_prev, flow, weight, flow_ref, conf_ref = reshape([real_slo, fake_slo, fake_slo_raw, fake_slo_ls, fake_slo_raw_ls, real_input_T, real_input_S, real_slo_prev, fake_slo_prev, flow, weight, flow_ref, conf_ref])

        _, _, self.height, self.width = real_slo.size()

        # flow loss
        if flow is not None:
            # similar to flownet flow        
            loss_F_Flow = self.criterionFlow(flow, flow_ref, conf_ref) * lambda_F       
            # warped prev image should be close to current image            
            real_slo_warp = self.resample(real_slo_prev, flow)                
            loss_F_Warp = self.criterionFlow(real_slo_warp, real_slo, conf_ref) * lambda_T
        ...
        #################### fake_B loss ####################        
        ### VGG + GAN loss 
        real_input = torch.cat([real_input_T, real_input_S], dim=1)
        loss_D_real, loss_D_fake, loss_G_GAN, loss_G_GAN_Feat = self.compute_loss_D(self.netD, real_input, real_slo, fake_slo)
        loss_PixelWise = self.criterionLO(fake_slo_ls, real_slo) * lambda_P
        ### Warp loss
        fake_slo_warp_ref = self.resample(fake_slo_prev, flow_ref)
        loss_G_Warp = self.criterionWarp(fake_slo, fake_slo_warp_ref.detach(), conf_ref) * lambda_T

        if fake_slo_raw is not None:     
            l_D_real, l_D_fake, l_G_GAN, l_G_GAN_Feat = self.compute_loss_D(self.netD, real_input, real_slo, fake_slo_raw)        
            l_PixelWise = self.criterionLO(fake_slo_raw_ls, real_slo) * lambda_P
            loss_G_GAN += l_G_GAN; loss_G_GAN_Feat += l_G_GAN_Feat
            loss_D_real += l_D_real; loss_D_fake += l_D_fake
            loss_PixelWise += l_PixelWise
    ...
    def get_all_skipped_frames(self, frames_all, real_B, fake_B, flow_ref, conf_ref, real_sfg, t_scales, tD, n_frames_load, i, flowNet):
        real_B_all, fake_B_all, flow_ref_all, conf_ref_all, real_sfg_all = frames_all
        if t_scales > 0:
            if self.opt.sparse_D:          
                ...
                real_sfg_all, real_sfg_skipped = get_skipped_frames(real_sfg_all, real_sfg, t_scales, tD)
                flow_ref_all, conf_ref_all, flow_ref_skipped, conf_ref_skipped = get_skipped_flows(flowNet, 
                    flow_ref_all, conf_ref_all, real_sfg_skipped, flow_ref, conf_ref, t_scales, tD)    
        frames_all = real_B_all, fake_B_all, flow_ref_all, conf_ref_all, real_sfg_all
        frames_skipped = real_B_skipped, fake_B_skipped, flow_ref_skipped, conf_ref_skipped, real_sfg_skipped
